...
class Eng(ast.NodeVisitor):
    ...
    def visit_Assign(self, n):
        ...
        if isinstance(val_node, ast.Call) and getattr(val_node.func, 'id', '') == 'input':
            if self.m == "cpp":
                ...
                self.buf.append(f"{self.pad()}char {v}[100];\n{self.pad()}scanf(\"%99s\", {v});")
            self.vmap[v] = "string"
            return
        ...
        if t == "string": decl = "string" if self.m == "cpp" else "const char*"
        elif t == "double": decl = "double"
        elif t == "bool": decl = "bool"
        elif t == "list": decl = "vector<int>" if self.m == "cpp" else "int"
        else: decl = "int"
    ...
    def visit_For(self, n):
        v = n.target.id
        lim = "0"
        if isinstance(n.iter, ast.Call) and getattr(n.iter.func, 'id', '') == 'range':
            lim = self.exp(n.iter.args[0])
...
class App(tk.Tk):
    ...
    def go(self):
        ...
        if m == "c" and os.path.exists(tbin):
            ...
            lst = ["tcc", "gcc"] if m == "c" else ["g++", "clang++"]
            for c in lst:
                try:
                    subprocess.run([c, "--version"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.DEVNULL, creationflags=flags, env=env)
                    cc = c
                    break
        ...
        try:
            if is_tcc:
                cmd = [cc, f"-I{os.path.join(tdir, 'include')}", f"-L{os.path.join(tdir, 'lib')}", "-run", src_f]
                res = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.DEVNULL, text=True, timeout=5, creationflags=flags, env=env)
                out = res.stdout if res.returncode == 0 else res.stderr
            elif cc == "tcc":
                res = subprocess.run([cc, "-run", src_f], stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.DEVNULL, text=True, timeout=5, creationflags=flags, env=env)
                out = res.stdout if res.returncode == 0 else res.stderr
            else:
                cres = subprocess.run([cc, "-O0", src_f, "-o", exe_f], stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.DEVNULL, text=True, creationflags=flags, env=env)
                if cres.returncode != 0:
                    self.set_log(f"Compilation Error:\n{cres.stderr}", col="#f38ba8")
                    return
                res = subprocess.run([exe_f], stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.DEVNULL, text=True, timeout=5, creationflags=flags, env=env)
                out = res.stdout

            self.set_log(out if out else "Done", col="#a6e3a1")
