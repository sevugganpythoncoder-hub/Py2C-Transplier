# Py2C-Transplier

It's A lightweight, high performance(kinda) desktop IDLE and AST transpiler that converts Python codes to C/C++ codes(your choice) and runs them as an exe

# Features and How it works:
-> It basically Transpiles Python code to standard C (using TinyCC cuz it's fast) and for C++ (using GCC/Clang).
-> Dark Themed Tkinter editor + auto indentation exists
->Its remotely fast in a sense as it uses TinyTCC and GCC to compiler/Transpile them.(Exact Timer Measurements are 0.919 sec for compiling and runnin C. similarily for C++ it takes 2.881)
-> Has support for AST syntax such as  variable assignments, math operations, basic loops ("range"), conditional blocks ("if/else/elif"), "while"` loops, lists, and user input.(Planning on expanding soon)
-
->P.S No need to download any other softwares as all of them are already bundled up as an executable

## Getting Started
1. Clone the repository.
3. Run "Py2C.vername" to launch the IDE.

